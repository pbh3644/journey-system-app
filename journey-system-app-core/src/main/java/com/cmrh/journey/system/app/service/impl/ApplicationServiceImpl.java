...
import com.cmrh.journey.system.common.utils.exception.BussinessException;
...
public class ApplicationServiceImpl extends BaseServiceImpl<ApplicationMapper, Application> implements ApplicationService {
    ...
    public void insert(Application application) {
        ...
        if (applicationMapper.uniquenessApplicationName(application.getApplicationNameEnglish()) != null) {
            ...
            new BussinessException("增加微服务失败！含微服务的名字重复，不允许增加。批量微服务的名字为：" + applicationNameEnglish);
        }
    ...
    public void insertBatch(List<Application> list) {
        for (Application application : list) {
            ...
            if (applicationMapper.uniquenessApplicationName(applicationNameEnglish) != null) {
                ...
                new BussinessException("批量增加微服务失败！其中包含微服务的名字重复，不允许批量增加。重复的微服务的名字为:" + applicationNameEnglish);
            }
        }
    ...
    public void update(Application application) {
        ...
        if (applicationMapper.uniquenessApplicationName(applicationNameEnglish) != null) {
            ...
            new BussinessException("修改微服务失败！这个微服务名字已经存在，不允许修改。重复的微服务名字为：" + applicationNameEnglish);
        }
    ...
    public void updateBatch(List<Application> list) {
        for (Application application : list) {
            ...
            if (applicationMapper.uniquenessApplicationName(applicationNameEnglish) != null) {
                ...
                new BussinessException("批量修改微服务的名字失败！其中包含微服务的名字重复，不允许批量修改。重复的微服务的名字为：" + applicationNameEnglish);
            }
        }
